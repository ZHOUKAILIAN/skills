...
- `figma-design-audit`
  - 只读 Figma 审计工作流
  - 负责完整遍历可见节点、分类节点、推导几何、识别业务来源和汇总阻塞问题
  - 不写 CSS、不改项目代码、不做最终还原质量评价
- `figma-1to1-ui-restoration`
  - Figma 1:1 UI 还原编排入口
  - 先路由到 `figma-design-audit` 产出审计事实，再用 `css-best-practices` 做实现策略
  - 需要验收时把只读复核交给 `figma-restoration-review`
